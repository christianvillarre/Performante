...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---------------- hero entrance ---------------- */
  if (window.gsap) {
    gsap.set(".vw-hero__copy, .vw-hero__side", { opacity: 0, y: 24 });
    gsap.timeline({ delay: 0.15, defaults: { ease: "power3.out", duration: 1 } })
      .to(".vw-hero__copy", { opacity: 1, y: 0 })
      .to(".vw-hero__side", { opacity: 1, y: 0 }, "-=0.7");
  }

  /* ---------------- scroll reveal ---------------- */
  const revealTargets = document.querySelectorAll(
    ".vw-service-card, .vw-process__item, .vw-testimonial-card, .vw-gallery__slide"
  );
